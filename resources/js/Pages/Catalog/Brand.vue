                        ...
                        <ul class="space-y-2">
                            <li v-for="category in categories" :key="category.id">
                                <Link
                                    ...
                                    class="block text-sm hover:text-indigo-600 text-gray-700"
                                >
                                    {{ category.name }}
...
const applyFilters = () => {
    ...
    router.visit(route('catalog.brand', { slug: props.brand.slug }), {
        data: query,
        preserveState: true,
        preserveScroll: true,
...
const toggleBrand = (brandId) => {
    if (brandId === props.brand.id.toString()) {
        return; // Не позволяем переключать текущий бренд
    }
